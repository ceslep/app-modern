...
if (empty($codigosRaw)) {
    ...
    echo json_encode(['error' => 'Códigos no proporcionados']);
    exit;
}

$codigos = array_map('trim', explode(',', $codigosRaw));
...
if (empty($html)) {
    ...
    header('Content-Type: application/json');
    echo json_encode(['error' => 'Ningún estudiante encontrado']);
    exit;
}
...
$mpdf->WriteHTML($html);
$mpdf->Output('Estudiantes_' . count($codigos) . '.pdf', 'I');
